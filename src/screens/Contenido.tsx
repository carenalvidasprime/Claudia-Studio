...
export function Contenido() {
  const app = useApp()
  const esFavoritos = app.pantalla === 'favoritos'
  const q = app.busquedaPiezas.trim().toLowerCase()

  // Base: piezas activas (fuera de la papelera). En Favoritos, solo las ★.
  const base = app.piezas.filter((p) => !p.descartada && (!esFavoritos || p.favorita))
  ...
  const cuenta = (e: Estado | 'Todas') =>
    base.filter(enCentro).filter((p) => e === 'Todas' || p.estado === e).length
  ...
  return (
    <section className="fade" style={sx('padding:28px 32px 60px')}>
      <div style={sx('display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap;margin-bottom:16px')}>
        <div style={sx('font-size:22px;font-weight:600;letter-spacing:-.02em')}>
          {esFavoritos ? 'Favoritos' : 'Contenido'}
        </div>
        <select
      ...
      {piezas.length === 0 ? (
        <div style={sx('background:#fff;border:1px dashed rgba(23,25,31,.18);border-radius:14px;padding:44px;text-align:center;color:rgba(23,25,31,.5);font-size:12.5px;line-height:1.6')}>
          {esFavoritos
            ? 'Aún no has marcado ninguna pieza como favorita. Pulsa la ★ en cualquier creatividad.'
            : app.piezas.filter((p) => !p.descartada).length === 0
            ...
            return (
              <div key={p.id} style={sx('position:relative')}>
                <button
                  ...
                  className="hv-card"
                  style={sx('display:block;width:100%;text-align:left;background:#fff;border:1px solid rgba(23,25,31,.08);border-radius:14px;overflow:hidden;cursor:pointer;padding:0;transition:box-shadow .16s')}
                >
                ...
                <button
                  onClick={() => void app.alternarFavorita(p.id)}
                  title={fav ? 'Quitar de favoritos' : 'Marcar como favorita'}
                  style={sx(
                    `position:absolute;top:8px;left:8px;width:26px;height:26px;border-radius:50%;border:none;display:grid;place-items:center;font-size:13px;cursor:pointer;background:rgba(23,25,31,.42);color:${fav ? 'oklch(0.8 0.16 75)' : '#fff'}`,
                  )}
                >
                  {fav ? '★' : '☆'}
                </button>
                <button
                  onClick={() => app.desecharPieza(p.id)}
                  title="Enviar a la papelera"
                  style={sx(
                    'position:absolute;top:8px;right:8px;width:26px;height:26px;border-radius:50%;border:none;display:grid;place-items:center;font-size:14px;cursor:pointer;background:rgba(23,25,31,.42);color:#fff',
                  )}
                >
                  ×
                </button>
